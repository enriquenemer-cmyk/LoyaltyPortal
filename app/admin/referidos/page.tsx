    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#150800 0%,#3D1200 50%,#150800 100%)', padding: '32px 16px', color: 'white', fontFamily: 'system-ui,sans-serif' }}>
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ ...card, display: 'flex', alignItems: 'center', gap: 16, marginBottom: 28, background: 'linear-gradient(135deg,rgba(249,115,22,0.18),rgba(194,65,12,0.12))', border: '1px solid rgba(249,115,22,0.30)' }}>
          <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'linear-gradient(135deg,#F97316,#C2410C)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 40, fontWeight: 900, color: '#F97316', lineHeight: 1 }}>{stats.total_referrals}</p>
          </div>
        </div>
              ...
              {stats.top_referrers.map((r, i) => (
                <div key={r.referral_code} style={{
                  ...
                  <div style={{
                    ...
                    background: i === 0 ? 'linear-gradient(135deg,#F97316,#C2410C)' : 'rgba(255,255,255,0.10)',
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <p style={{ fontSize: 22, fontWeight: 900, color: i === 0 ? '#F97316' : 'white', lineHeight: 1 }}>{r.count}</p>
                ...
                <tbody>
                  {stats.chains.map((c, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                      ...
                      <td style={{ padding: '10px 12px', fontFamily: 'monospace', color: '#F97316', letterSpacing: '0.06em' }}>{c.referral_code}</td>
