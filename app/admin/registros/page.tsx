...
<head>
  <meta charset="UTF-8" />
  <title>Reporte de Cobros — 3E</title>
  <style>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              ...
              <p className="text-orange-200/70 mt-1.5 text-sm">Historial completo de premios reclamados</p>
            </div>
          </div>
        </div>
      </div>
                ...
                <div className="mobile-cards sm:hidden divide-y divide-[#F0EDE8]">
                  {filtered.map((claim) => (
                    <div key={claim.id} className="flex gap-0 hover:bg-[#faf7f5] transition-colors stagger-item min-h-[64px]">
                      ...
                      <div className={`w-1.5 shrink-0 rounded-none ${claim.status === 'delivered' ? 'bg-orange-400' : 'bg-orange-400'}`} />
                            ...
                            <Fragment key={claim.id}>
                              ...
                              >
                                ...
                                <td className={`px-5 ${compact ? 'py-2' : 'py-4'}`}>
                                  <span className="inline-flex items-center bg-emerald-50 text-[#1a6b3c] text-xs font-semibold px-2.5 py-1 rounded-full border border-indigo-200">
                                    {claim.prize_name}
                                  </span>
                                </td>
                                <td className={`px-5 ${compact ? 'py-2' : 'py-4'}`}>
                                    ...
                                    {resendSuccess === claim.id ? (
                                      ...
                                      <Tooltip text="Reenviar QR al correo del cliente">
                                        <button
                                          ...
                                          className="inline-flex items-center gap-1.5 text-xs font-semibold text-orange-700 border border-orange-200 bg-orange-50 hover:bg-orange-100 px-2.5 py-1.5 rounded-lg transition-colors disabled:opacity-50"
                                        >
                                          {resending === claim.id ? (
