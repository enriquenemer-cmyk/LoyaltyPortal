  ...
  return (
    ...
    >
      <div className="bg-white rounded-2xl border border-[#E8E3DC] shadow-2xl w-full max-w-sm overflow-hidden">
        {/* Header */}
        <div style={{ background: 'linear-gradient(135deg,#E8521A,#C2410C)', padding: '20px 22px' }} className="flex items-center justify-between">
          <div>
...
export default function RestaurantesPage() {
  ...
  const [form, setForm] = useState({ name: '', address: '', phone: '', accent_color: '#E8521A', google_maps_url: '' });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setForm({ name: '', address: '', phone: '', accent_color: '#E8521A', google_maps_url: '' });
      setShowForm(false);
    } catch {
          ...
          <SharedEmptyState
            ...
            action={
              <button
                ...
                style={{ background: 'linear-gradient(135deg,#E8521A,#C2410C)', boxShadow: '0 4px 14px rgba(232,82,26,0.3)' }}
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                </svg>
