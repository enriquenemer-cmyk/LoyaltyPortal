  ...
  const { rows: budget } = await pool.query(
    `SELECT b.category, b.budgeted::text,
     ...
     LEFT JOIN accounting_entries ae
       ON ae.restaurant_id IS NOT DISTINCT FROM b.restaurant_id
       AND ae.category = b.category
       AND ae.type = 'expense'
       AND TO_CHAR(ae.date, 'YYYY-MM') = b.month
     WHERE b.restaurant_id IS NOT DISTINCT FROM $1 AND b.month = $2
...
export async function POST(req: NextRequest) {
  ...
  await pool.query(
    `INSERT INTO budget_items (restaurant_id, month, category, budgeted)
     VALUES ($1,$2,$3,$4)
     ON CONFLICT (COALESCE(restaurant_id, ''), month, category) DO UPDATE SET budgeted=$4`,
...
export async function DELETE(req: NextRequest) {
  ...
  const pool = getPool();
  await pool.query(
    `DELETE FROM budget_items WHERE restaurant_id IS NOT DISTINCT FROM $1 AND month=$2 AND category=$3`,
