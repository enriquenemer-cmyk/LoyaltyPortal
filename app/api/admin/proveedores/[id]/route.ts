...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const pool = getPool();
  await pool.query(`DELETE FROM suppliers WHERE id=$1 AND restaurant_id IS NOT DISTINCT FROM $2`, [id, restaurantId]);
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { rows } = await pool.query(
    `UPDATE suppliers SET name=$1, contact_name=$2, contact_phone=$3, contact_email=$4, notes=$5 WHERE id=$6 AND restaurant_id IS NOT DISTINCT FROM $7 RETURNING *`,
