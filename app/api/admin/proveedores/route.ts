...
export async function POST(req: NextRequest) {
  ...
  const pool = getPool();
  const { rows } = await pool.query(
    `INSERT INTO suppliers (restaurant_id, name, contact_name, phone, email, notes)
     VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
    [session.restaurantId, body.name, body.contact_name ?? null, body.phone ?? null, body.email ?? null, body.notes ?? null]
  );
