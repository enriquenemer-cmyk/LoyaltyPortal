...
type VerifyResult =
  | { status: 'valid'; play: { id: string; folio: string; full_name: string; phone: string; prize_name: string; bundle_name: string; played_at: string } }
...
export default function VerificarJuegoPage() {
  ...
  async function startQRScan() {
    ...
    try {
      ...
      const controls = await reader.decodeFromVideoDevice(undefined, videoEl, async (res) => {
        ...
        const match = text.match(/PT-[A-Z0-9]{6,}/i);
  ...
  async function handleVerify(folioOverride?: string) {
    const target = (folioOverride ?? folio).toUpperCase().trim();
    if (!target) return;
    setLoading(true);
    setResult(null);
    setError('');
    ...
    try {
      const res = await fetch(`/api/game-bundles/verify?folio=${encodeURIComponent(target)}`);
  ...
  async function handleRedeem() {
    ...
    setError('');
    try {
      const res = await fetch('/api/game-bundles/redeem', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ folio: result.play.folio, cashier_name: cashierName.trim() }),
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  function handleFolioChange(val: string) {
    setFolio(val.toUpperCase());
    setResult(null);
    setRedeemSuccess(false);
    setError('');
  }
      ...
      <div className="sticky top-0 z-20 shadow-md" style={{ background: `linear-gradient(135deg, #F97316, #EA580C)` }}>
        <div className="max-w-lg mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="text-white font-extrabold text-base leading-tight">Verificar Premio de Juego</p>
            <p className="text-white/70 text-xs">Folio de campaña de juego · Canjear</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-5">
        ...
        <div className="bg-white rounded-3xl border border-[#E8E3DC] overflow-hidden" style={{ boxShadow: '0 1px 2px rgba(28,25,23,0.04), 0 8px 32px rgba(28,25,23,0.08)' }}>
          <div className="px-5 py-4 border-b border-[#E8E3DC]">
            <h2 className="text-[#1C1917] font-extrabold text-base">Verificar Folio</h2>
            <p className="text-[#a8a29e] text-xs mt-0.5">Ingresa el folio o escanea el comprobante del cliente</p>
          </div>
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                title={scanning ? 'Detener camara' : 'Escanear'}
        ...
        {redeemSuccess && (
          <div className="bg-white rounded-3xl border border-emerald-200 overflow-hidden" style={{ boxShadow: '0 8px 32px rgba(16,185,129,0.12)' }}>
            <div className="p-8 text-center">
              ...
              <p className="text-emerald-600 text-sm">El folio fue marcado como entregado exitosamente.</p>
              <button onClick={() => { setFolio(''); setRedeemSuccess(false); setResult(null); }} className="mt-5 text-emerald-600 font-bold text-sm underline">
        ...
        {result?.status === 'valid' && (
          <div className="bg-white rounded-3xl border border-emerald-200 overflow-hidden" style={{ boxShadow: '0 8px 32px rgba(16,185,129,0.10)' }}>
            <div className="px-5 py-4 bg-emerald-50 border-b border-emerald-200 flex items-center gap-3">
              ...
              <div>
                <p className="text-emerald-800 font-extrabold text-base">Folio válido</p>
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-[#FAFAF9] border border-[#E8E3DC] rounded-2xl px-4 py-3">
                  ...
                  <p className="text-[#1C1917] font-bold text-sm">{result.play.full_name}</p>
                  <p className="text-stone-400 text-xs">{result.play.phone}</p>
                </div>
                <div className="bg-[#FAFAF9] border border-[#E8E3DC] rounded-2xl px-4 py-3">
                  <p className="text-stone-400 text-[10px] font-bold uppercase tracking-widest mb-1">Campaña</p>
                  <p className="text-[#1C1917] font-bold text-sm">{result.play.bundle_name}</p>
                </div>
                <div className="bg-[#FAFAF9] border border-[#E8E3DC] rounded-2xl px-4 py-3">
                  <p className="text-stone-400 text-[10px] font-bold uppercase tracking-widest mb-1">Premio</p>
                  <p className="text-[#1C1917] font-bold text-sm">{result.play.prize_name}</p>
                </div>
                <div className="bg-[#FAFAF9] border border-[#E8E3DC] rounded-2xl px-4 py-3">
                  <p className="text-stone-400 text-[10px] font-bold uppercase tracking-widest mb-1">Jugado el</p>
                  <p className="text-[#1C1917] font-bold text-xs">{formatDate(result.play.played_at)}</p>
                </div>
              </div>
          ...
          <div className="bg-white rounded-3xl border border-red-200 overflow-hidden" style={{ boxShadow: '0 8px 32px rgba(239,68,68,0.10)' }}>
            <div className="px-5 py-4 bg-red-50 border-b border-red-200 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-red-500 text-xs">Este folio ya fue utilizado anteriormente</p>
              </div>
            </div>
        ...
        {result?.status === 'not_found' && (
          <div className="bg-white rounded-3xl border border-red-200 overflow-hidden" style={{ boxShadow: '0 8px 32px rgba(239,68,68,0.10)' }}>
            <div className="p-8 text-center">
              ...
              <p className="text-red-700 font-extrabold text-lg">Folio no encontrado</p>
              <p className="text-red-400 text-sm mt-1">Verifica que el folio esté bien escrito.</p>
            </div>
          </div>
        )}
