...
function ReferidosContent() {
  ...
  const handleWhatsApp = () => {
    const msg = encodeURIComponent(
      `¡Hola! Te invito a ganar premios gratis en Super Tierra. Regístrate con mi link y ambos ganamos: ${referralLink}`
    );
    window.open(`https://wa.me/?text=${msg}`, '_blank');
  };

  const handleSMS = () => {
    const msg = encodeURIComponent(`¡Gana premios en Super Tierra! Usa mi link: ${referralLink}`);
